...
/**
 * Class for toggling reviews.
 */
class ReviewsToggle {
  constructor () {
    this.reviewsContainer = $('#reviews_container');
    this.toggleReviewsSpan = $('#toggle_reviews');

    // Attach the click event handler
    this.toggleReviewsSpan.click(this.toggle.bind(this));
  }

  toggle () {
    if (this.toggleReviewsSpan.text() === 'show') {
      // Fetch and display reviews
      $.get('/101-hbnb/reviews', (data) => {
        this.reviewsContainer.html(data);
        this.toggleReviewsSpan.text('hide');
      });
    } else {
      // Hide reviews
      this.reviewsContainer.empty();
      this.toggleReviewsSpan.text('show');
    }
  }
}

/**
 * Document ready function to initialize the API status checker, place loader, and toggle reviews functionality.
 */
$(document).ready(function () {
  ...
  const reviewsToggle = new ReviewsToggle();
});
